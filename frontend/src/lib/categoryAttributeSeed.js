export const CATEGORY_ATTRIBUTE_SEED = {
  mobile: [
...
export const buildSeedAttributeConfigs = (categoryType) => {
  const key = toCode(categoryType);
  const seeds = CATEGORY_ATTRIBUTE_SEED[key] || [];
  return seeds.map((seed) => {
    const code = toCode(`${key}_${seed.name}`);
    return {
      attribute_id: null,
      seed_code: code,
      seed_name: seed.name,
      ...
      is_required: false,
      is_visible: true,
      is_filterable: seed.type === "multi-select",
      is_searchable: seed.type !== "number",
      sort_order: 0,
      value: "",
    };
  });
