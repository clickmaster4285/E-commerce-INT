...
// ✅ CHANGE 1: adminCategoryApi import
...
export default function CategoriesPage() {
  ...
  // ==========================================
  // ✅ CHANGE 2: adminCategories queryKey + adminCategoryApi.getAll
  // ==========================================
  const {
    data: categories = [],
    isLoading: loading,
  ...
  const fetchNextCategoryCode = async () => {
    try {
      setLoadingCode(true);
      // ✅ CHANGE 3: adminCategoryApi.getNextCode
      const response = await adminCategoryApi.getNextCode();
      ...
      if (
        ...
        console.error(
          "Failed to fetch next category code, using fallback:",
          err,
        );
      }

      const codedCategories = categories.filter(
        (c) => c.category_code && /^CAT-\d+$/i.test(c.category_code),
      );
  ...
  // ✅ CHANGE 4: adminCategoryApi.create/update + ✅ CHANGE 5: adminCategories invalidate
  const categoryMutation = useMutation({
    mutationFn: ({ data, id }) =>
      id ? adminCategoryApi.update(id, data) : adminCategoryApi.create(data),
  ...
  // ✅ CHANGE 6: adminCategoryApi.delete + ✅ CHANGE 7: adminCategories invalidate
  const deleteMutation = useMutation({
    mutationFn: (ids) => Promise.all(ids.map((id) => adminCategoryApi.delete(id))),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["adminCategories"] });
      setSelectedIds([]);
  ...
  const sortedCategories = useMemo(() => {
    ...
    arr.sort((a, b) => {
      ...
      switch (sortConfig.key) {
        case "code":
          va = a.category_code?.toLowerCase() || "";
          vb = b.category_code?.toLowerCase() || "";
          break;
        case "name":
  ...
  const allSelected =
    ...
    paginatedCategories.every((c) => selectedIds.includes(c._id));
  ...
  const handleDelete = (category) =>
    setDeleteTarget({ categories: [category] });
  const handleBulkDelete = () =>
    setDeleteTarget({
      categories: categories.filter((c) => selectedIds.includes(c._id)),
    });
        ...
        {totalCategories > 20 && (
          ...
          >
            <p className="text-[12px]" style={{ color: "var(--text-muted)" }}>
              Showing {startIndex + 1}-{Math.min(endIndex, totalCategories)} of{" "}
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      disabled={
                        isSubmitting || loadingCode || !!editingCategory
                      }
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-sm font-semibold">
                  {deleteTarget.categories.length === 1
                ...
                <p
                  ...
                  style={{ color: "var(--text-muted)" }}
                >
                  This action cannot be undone. The category(s) will be
                  permanently removed.
                </p>
              </div>
            </div>
            ...
            >
              ...
              <div className="min-w-0">
                <p className="text-sm font-medium truncate">
                  {deleteTarget.categories[0].name}
                </p>
                <p
                ...
                >
                  {deleteTarget.categories.length === 1
                    ? deleteTarget.categories[0].category_code || "—"
                    : `+ ${deleteTarget.categories.length - 1} more`}
                </p>
              </div>
            </div>
