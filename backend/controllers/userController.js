...
const createUser = async (req, res) => {
  try {
    ...
    // ✅ FIX: Sirf staff/admin allow karein registration par
    const allowedRoles = ["staff", "admin"];
    const finalRole = allowedRoles.includes(role?.toLowerCase()) ? role.toLowerCase() : "staff";
...
const refreshAccessToken = async (req, res) => {
  try {
    ...
    // ✅ Refresh token par bhi role check zaroori hai
    if (!["admin", "staff", "manager"].includes(user.role)) {
...
// ==========================================
// UPDATE PROFILE (REST API - FIXED)
// ==========================================
const updateProfile = async (req, res) => {
  try {
    const { name, email, phone, permissions, preferences } = req.body;
    const userId = req.user._id;

    // Fetch old data for activity tracking
    ...
    const trackedFields = ["name", "email", "phone"];
    const changes = getChanges(oldUser.toObject(), updatedUser.toObject(), trackedFields);

    if (changes.length > 0) {
      const io = req.io || getIO();
      await pushGlobalActivity(io, {
        action: `${updatedUser.name} updated profile details`,
        category: "Authentication",
        performedBy: userId,
        performedByName: updatedUser.name,
        details: { changes },
      }, userId);
    }
...
// ==========================================
// ✅ TOGGLE 2FA (MISSING FUNCTION ADDED)
// ==========================================
...
const updateProfileInfo = async (req, res) => {
  try {
    ...
    // ✅ FIX: Fetch user first to avoid 'user' undefined error
...
module.exports = {
  ...
  getProfileInfo, updateProfileInfo, changePasswordSocket, googleLogin,
};